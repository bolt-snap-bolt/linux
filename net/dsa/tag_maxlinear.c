...
/* Set this to 1 for verbose kernel log output */
#define RCC_ML_TAG_DBG_PRINT		(0)
...
#if RCC_ML_TAG_DBG_PRINT
	printk("!RCC: Tx Ingress Tag for port %u", dp->index);
	printk("!RCC: \t\t 0x%X %X %X %X", \
		ml_ingress_tag[0], ml_ingress_tag[1], \
		ml_ingress_tag[2], ml_ingress_tag[3]);
	printk("!RCC: \t\t 0x%X %X %X %X", \
		ml_ingress_tag[4], ml_ingress_tag[5], \
		ml_ingress_tag[6], ml_ingress_tag[7]);
#endif
...
#if RCC_ML_TAG_DBG_PRINT
	printk("!RCC: Rx Egress Tag:");
	printk("!RCC: \t\t 0x%X %X %X %X", \
		ml_egress_tag[0], ml_egress_tag[1], \
		ml_egress_tag[2], ml_egress_tag[3]);
	printk("!RCC: \t\t 0x%X %X %X %X", \
		ml_egress_tag[4], ml_egress_tag[5], \
		ml_egress_tag[6], ml_egress_tag[7]);
#endif

	if ((ml_egress_tag[0] != ML_SPECIAL_TAG_ETHERTYPE_B1)
		|| (ml_egress_tag[1] != ML_SPECIAL_TAG_ETHERTYPE_B2)) {
#if RCC_ML_TAG_DBG_PRINT
		printk("!RCC: ERROR: expected egress special tag Ethertype!");
#endif
		return NULL;
	}
	
	/* Get source port information */
	port = (ml_egress_tag[2] & ML_RX_INGRESS_PORT_NUM_MASK) >> ML_RX_INGRESS_PORT_NUM_SHIFT;
	skb->dev = dsa_master_find_slave(dev, 0, port);
	if (!skb->dev) {
#if RCC_ML_TAG_DBG_PRINT
		printk("!RCC: ERROR: Invalid source port of %u", port);
#endif
		return NULL;
	}
